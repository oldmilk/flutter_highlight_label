...
/// Where to show a [HighlightLabel].
///
/// The start and end locations are relative to the ambient [Directionality]
/// (which can be overridden by [HighlightLabel.layoutDirection]).
enum HighlightLabelLocation {
  /// Show the banner in the top-right corner when the ambient [Directionality]
  /// (or [HighlightLabel.layoutDirection]) is [TextDirection.rtl] and in the top-left
  ...
  /// (or [HighlightLabel.layoutDirection]) is [TextDirection.rtl] and in the top-right
  /// corner when the ambient [Directionality] is [TextDirection.ltr].
  topEnd,

  /// Show the banner in the bottom-right corner when the ambient
  /// [Directionality] (or [HighlightLabel.layoutDirection]) is [TextDirection.rtl] and
...
/// Paints a [HighlightLabel].
class HighlightLabelPainter extends CustomPainter {
  /// Creates a banner painter.
  ///
  /// The [message], [textDirection], [location], and [layoutDirection]
  /// arguments must not be null.
  HighlightLabelPainter({
  ...
  final HighlightLabelLocation location;
  ...
  Paint _paintShadow;
  Paint _paintHighlightLabel;

  void _prepare() {
    _paintShadow = _shadow.toPaint();
    _paintHighlightLabel = Paint()
      ..color = color;
  ...
  @override
  bool shouldRepaint(HighlightLabelPainter oldDelegate) {
  ...
  double _translationX(double width) {
    ...
    switch (layoutDirection) {
      case TextDirection.rtl:
        switch (location) {
          case HighlightLabelLocation.bottomEnd:
            return _kBottomOffset;
          case HighlightLabelLocation.topEnd:
            return 0.0;
          case HighlightLabelLocation.bottomStart:
            return width - _kBottomOffset;
          case HighlightLabelLocation.topStart:
            return width;
        }
        break;
...
class HighlightLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomPaint(
      foregroundPainter: HighlightLabelPainter(
        message: message,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(EnumProperty<HighlightLabelLocation>('location', location));
